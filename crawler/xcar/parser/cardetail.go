...
var nameRe = regexp.MustCompile(`<title>【(.*)】.*</title>`)
...
func ParseCarDetail(contents []byte, url string) engine.ParserResult {

	id := extractString([]byte(url), urlRe)

	log.Println("car name:", extractString(contents, nameRe))
	...
	priceRe, err := regexp.Compile(fmt.Sprintf(priceReTmpl, regexp.QuoteMeta(id)))
	if err == nil {
